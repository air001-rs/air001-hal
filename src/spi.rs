    ...
    SPI: Deref<Target = SpiRegisterBlock>,
{
    ...
    pub fn into_8bit_width(self) -> Spi<SPI, SCKPIN, MISOPIN, MOSIPIN, EightBit> {
        ...
        self.spi
            .cr2
            .write(|w| w.frxth().set_bit().ds().clear_bit().ssoe().clear_bit());
    ...
    pub fn into_16bit_width(self) -> Spi<SPI, SCKPIN, MISOPIN, MOSIPIN, SixteenBit> {
        ...
        self.spi
            .cr2
            .write(|w| w.frxth().set_bit().ds().set_bit().ssoe().clear_bit());
    ...
    fn send_u8(&mut self, byte: u8) {
        let write_ptr = core::ptr::addr_of!(self.spi.dr) as *mut u8;
        // note(write_volatile) write to register owned by this spi struct
        unsafe { ptr::write_volatile(write_ptr, byte) }
    }
    ...
    fn send_u16(&mut self, byte: u16) {
        // See https://github.com/stm32-rs/stm32f7xx-hal/pull/207
        // NOTE(write_volatile) write to register owned by this spi struct
        let write_ptr = core::ptr::addr_of!(self.spi.dr) as *mut u16;
        unsafe { ptr::write_volatile(write_ptr, byte) }
    }
