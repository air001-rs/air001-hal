        ...
        $(
                ...
                T: Into<Hertz>,
            {
                ...
                unsafe {
                    if PINS::C1 {
                        tim.ccmr1_output()
                           .modify(|_, w| w.oc1pe().set_bit().oc1m().bits(mode1));
                    }
                    if PINS::C2 {
                        tim.ccmr1_output()
                           .modify(|_, w| w.oc2pe().set_bit().oc2m().bits(mode1) );
                    }
                    if PINS::C3 {
                        tim.ccmr2_output()
                           .modify(|_, w| w.oc3pe().set_bit().oc3m().bits(mode1) );
                    }
                    if PINS::C4 {
                        tim.ccmr2_output()
                           .modify(|_, w| w.oc4pe().set_bit().oc4m().bits(mode1) );
                    }
                }
            ...
            impl hal::PwmPin for PwmChannels<$TIMX, C3> {
                ...
                //NOTE(unsafe) atomic write with no side effects
                fn set_duty(&mut self, duty: u16) {
                    unsafe { (*$TIMX::ptr()).ccr3.write(|w| w.ccr3().bits(duty.into())) }
                }
            }
            ...
            impl hal::PwmPin for PwmChannels<$TIMX, C4> {
                ...
                //NOTE(unsafe) atomic write with no side effects
                fn set_duty(&mut self, duty: u16) {
                    unsafe { (*$TIMX::ptr()).ccr4.write(|w| w.ccr4().bits(duty.into())) }
                }
            }
        )+
    };
